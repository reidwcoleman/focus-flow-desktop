      ...
      <div className="relative min-h-[400px] bg-gradient-to-b from-dark-bg-tertiary/30 to-dark-bg-surface/30 rounded-xl p-4 border border-dark-border-glow">
        ...
        <div className="relative space-y-6 py-4">
          {sortedActivities.map((activity, index) => {
            const position = getTimelinePosition(activity.start_time)

            return (
              <div
                key={activity.id}
                className="relative pl-16 animate-stagger-fade-in"
                style={{
                  animationDelay: `${index * 0.1}s`,
                  ...(position !== null && { position: 'absolute', top: `${position}%` })
                }}
              >
                {/* Timeline dot */}
                <div className={`absolute left-5 w-8 h-8 rounded-full bg-gradient-to-br ${getTypeColor(activity.activity_type)} border-2 border-white shadow-lg flex items-center justify-center text-lg z-10`}>
                  {getTypeIcon(activity.activity_type)}
                </div>

                {/* Activity card */}
                <div className="group relative overflow-hidden bg-dark-bg-secondary/50 backdrop-blur-sm rounded-xl p-4 border border-white/10 hover:border-primary-500/30 hover:scale-[1.02] transition-all duration-300 shadow-[0_4px_16px_0_rgba(0,0,0,0.2)]">
                  {/* Gradient overlay */}
