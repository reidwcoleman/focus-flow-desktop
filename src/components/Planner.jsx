...
const Planner = () => {
  ...
  return (
    <div className="space-y-6 pb-8 animate-fadeIn max-w-[1800px] mx-auto">
      {/* Header with Stats */}
      <div>
        <div className="flex items-start justify-between mb-4">
          <div>
            <h1 className="text-4xl font-bold text-dark-text-primary mb-2">Smart Calendar</h1>
            <p className="text-lg text-dark-text-secondary">AI-powered activity planning</p>
          </div>
          <div className="text-right">
            <div className="text-3xl font-bold text-primary-500">{activities.length}</div>
            <div className="text-sm text-dark-text-muted">activities</div>
          </div>
        </div>
        ...
        {loading ? (
          ...
          <div className="flex gap-4 mt-4 animate-fadeIn">
            <div className="flex-1 bg-dark-bg-secondary rounded-2xl p-5 border border-dark-border-glow">
              <div className="text-base text-dark-text-muted mb-1">Total Hours</div>
              <div className="text-3xl font-bold text-dark-text-primary">{getTotalHours().toFixed(1)}h</div>
            </div>
            <div className="flex-1 bg-dark-bg-secondary rounded-2xl p-5 border border-dark-border-glow">
              <div className="text-base text-dark-text-muted mb-1">Completed</div>
              <div className="text-3xl font-bold text-green-500">
                {activities.filter(a => a.is_completed).length}
              </div>
            </div>
            <div className="flex-1 bg-dark-bg-secondary rounded-2xl p-5 border border-dark-border-glow">
              <div className="text-base text-dark-text-muted mb-1">Pending</div>
      ...
      {/* AI Activity Input */}
      <div className="bg-gradient-to-br from-primary-500/10 via-dark-bg-secondary to-accent-purple/10 rounded-2xl p-6 border border-dark-border-glow shadow-dark-soft-lg">
        {/* Success/Error Messages */}
        {success && (
          <div className="mb-4 p-4 rounded-xl bg-green-500/10 border border-green-500/30 animate-fadeIn">
            <p className="text-green-400 text-base font-medium">{success}</p>
          </div>
        )}
        {error && (
        ...
        <div className="flex gap-3">
          <input
            ...
            className="flex-1 px-5 py-4 text-lg rounded-xl bg-dark-bg-tertiary border border-dark-border-glow text-dark-text-primary placeholder:text-dark-text-muted focus:outline-none focus:border-primary-500 focus:ring-2 focus:ring-primary-500/20 transition-all"
            disabled={aiProcessing}
          />
          <button
            onClick={handleAiCreate}
            disabled={aiProcessing || !aiInput.trim()}
            className="px-6 py-4 bg-gradient-to-r from-primary-500 to-accent-cyan text-white text-base font-semibold rounded-xl hover:shadow-glow-cyan transition-all disabled:opacity-50 disabled:cursor-not-allowed active:scale-95 flex items-center gap-2"
          >
            {aiProcessing ? (
        ...
        {/* Pro Tip */}
        <div className="mt-4 flex items-start gap-2 text-sm text-dark-text-muted">
          <svg className="w-4 h-4 flex-shrink-0 mt-0.5 text-primary-500" fill="currentColor" viewBox="0 0 20 20">
        ...
        <button
          ...
          className="mt-3 flex items-center gap-2 text-sm text-dark-text-secondary hover:text-primary-500 transition-colors"
        >
          <svg
        ...
        {showExamples && (
          <div className="mt-3 grid grid-cols-4 gap-3 animate-fadeIn">
            <button
              onClick={() => { setAiInput("Study chemistry tomorrow at 3pm for 2 hours"); setShowExamples(false); }}
              className="text-sm px-4 py-3 rounded-lg bg-dark-bg-tertiary border border-dark-border-subtle text-dark-text-muted hover:text-primary-500 hover:border-primary-500/50 transition-all text-left"
            >
              Study tomorrow 3pm
            </button>
            <button
      ...
      <div className="space-y-3">
        ...
        <div className="flex gap-3">
          <button
            ...
            className={`flex-1 py-4 px-5 rounded-xl font-semibold text-base transition-all active:scale-95 ${
              viewMode === 'calendar'
        ...
        <div className="flex gap-2 overflow-x-auto pb-1 scrollbar-hide">
          {activityTypes.map(type => (
            <button
              ...
              className={`px-5 py-2.5 rounded-xl text-sm font-semibold whitespace-nowrap transition-all active:scale-95 ${
                filterType === type
      ...
      {viewMode === 'calendar' && (
        loading ? (
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 animate-fadeIn">
            ...
            <div
              className="bg-dark-bg-secondary rounded-2xl p-6 border border-dark-border-glow shadow-dark-soft-md"
            ...
            >
              <div className="flex items-center justify-between mb-6">
                ...
                <div className="flex gap-2">
                  <button
                    onClick={goToToday}
                    className="px-4 py-2.5 rounded-xl bg-primary-500/10 border border-primary-500/30 text-primary-500 text-sm font-semibold hover:bg-primary-500/20 transition-all active:scale-95"
                  >
                    Today
                  </button>
                  <button
                    onClick={previousMonth}
                    className="w-10 h-10 rounded-xl bg-dark-bg-tertiary border border-dark-border-glow text-dark-text-primary hover:border-primary-500 transition-all active:scale-95"
                  >
              ...
              <div className="grid grid-cols-7 gap-2 mb-2">
                {dayNames.map(day => (
                  <div key={day} className="text-center text-sm font-bold text-dark-text-muted py-2">
                    {day}
                  </div>
                ))}
              </div>
                ...
                {getDaysInMonth().map((day, index) => {
                  ...
                  return (
                    <button
                      ...
                      disabled={!day}
                      className={`aspect-square rounded-xl p-2 transition-all text-center ${
                        !day
                          ? 'invisible'
                          : isSelected(day)
                    ...
                    >
                      {day && (
                        <div className="flex flex-col items-center justify-center h-full">
                          <span className={`text-base font-semibold ${isSelected(day) ? 'text-white' : ''}`}>
                            {day}
                          </span>
                          {hasActivities && (
            ...
            {loading ? (
              <div className="bg-dark-bg-secondary rounded-2xl p-6 border border-dark-border-glow shadow-dark-soft-md">
                <div className="h-6 bg-dark-bg-tertiary rounded w-48 mb-4 animate-pulse"></div>
              ...
              <div className="bg-dark-bg-secondary rounded-2xl p-6 border border-dark-border-glow shadow-dark-soft-md">
                ...
                {getFilteredActivities(dayActivities).length === 0 ? (
                  <div className="text-center py-12">
                    ...
                    <p className="text-dark-text-secondary text-base font-medium">No activities</p>
                    <p className="text-dark-text-muted text-sm mt-1">Use AI to create one</p>
                  </div>
                ) : (
                  <div className="space-y-4">
                      ...
                      >
                        <div className="flex items-start gap-4">
                          <button
                            ...
                            className={`mt-1 w-6 h-6 flex-shrink-0 rounded border-2 flex items-center justify-center transition-all ${
                              activity.is_completed
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="flex items-start justify-between gap-3 mb-2">
                              ...
                              {activity.ai_generated && (
                                <div className="flex-shrink-0 flex items-center px-2 py-1 rounded bg-primary-500/10 border border-primary-500/30">
                            ...
                            <div className="flex flex-wrap items-center gap-3 text-sm text-dark-text-muted">
                              ...
                              <span className={`px-2.5 py-1 rounded-lg ${getTypeColor(activity.activity_type)} text-white font-medium text-xs`}>
                                {activity.activity_type}
                              </span>
                            </div>

                            {activity.description && (
                              <p className="text-sm text-dark-text-secondary mt-3 line-clamp-2">{activity.description}</p>
                            )}
                          </div>

                          <button
                            onClick={() => handleDeleteActivity(activity.id)}
                            className="flex-shrink-0 w-9 h-9 rounded-xl bg-red-500/10 border border-red-500/30 text-red-400 hover:bg-red-500/20 transition-all active:scale-95"
                          >
      ...
      {viewMode === 'upcoming' && (
        loading ? (
          <div className="bg-dark-bg-secondary rounded-2xl p-6 border border-dark-border-glow shadow-dark-soft-md">
            <div className="h-6 bg-dark-bg-tertiary rounded w-32 mb-4 animate-pulse"></div>
            <div className="space-y-4">
              <ActivitySkeleton />
              <ActivitySkeleton />
              <ActivitySkeleton />
          ...
          <div className="bg-dark-bg-secondary rounded-2xl p-6 border border-dark-border-glow shadow-dark-soft-md animate-fadeIn">
          <h3 className="text-2xl font-bold text-dark-text-primary mb-5">Next 7 Days</h3>
          ...
          {getFilteredActivities(getUpcomingActivities()).length === 0 ? (
            <div className="text-center py-12">
              ...
              <p className="text-dark-text-secondary text-base font-medium">No upcoming activities</p>
              <p className="text-dark-text-muted text-sm mt-1">Create one using AI above</p>
            </div>
          ) : (
            <div className="space-y-4">
                  ...
                  <div key={activity.id}>
                    {showDateHeader && (
                      <div className="flex items-center gap-3 mb-3 mt-5 first:mt-0">
                        ...
                        <div className="text-sm font-bold text-dark-text-secondary">
